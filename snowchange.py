  ...
  for database_folder in os.scandir(root_folder):
    if database_folder.is_dir():
      ...
      if append_environment_name:
        snowflake_database_name = snowflake_database_name + '_' + environment_name.upper()
...
def apply_change_script(database, script, verbose):
  ...
  query = "INSERT INTO SNOWCHANGE.CHANGE_HISTORY (VERSION, DESCRIPTION, TYPE, SCRIPT, CHECKSUM, INSTALLED_BY, INSTALLED_ON, EXECUTION_TIME, STATUS) values ('{0}','{1}','{2}','{3}','{4}','{5}',CURRENT_TIMESTAMP,{6},'{7}');".format(script['script_version'], script['script_description'], script['script_type'], script['script_name'], checksum, os.environ["SNOWFLAKE_USER"], execution_time, status)
  execute_snowflake_query(database, query, verbose)


if __name__ == '__main__':
  parser = argparse.ArgumentParser("Apply schema changes to a Snowflake data warehouse. Full readme at https://github.com/jamesweakley/snowchange")
