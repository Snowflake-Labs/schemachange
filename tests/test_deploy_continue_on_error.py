...
def test_fail_fast_path_does_not_double_log(tmp_path: Path):
    """In the default fail-fast path the outer except must not add an extra log line.
    The error is already logged inside apply_change_script / execute_snowflake_query."""
    (tmp_path / "R__one.sql").write_text("select 1;")
    config = DeployConfig.factory(
        config_file_path=tmp_path / "config.yml",
        root_folder=tmp_path,
    ...
    session.change_history_table = ChangeHistoryTable()
    session.get_script_metadata.return_value = ({}, {}, None)
    session.apply_change_script.side_effect = Exception("boom")
    ...
    outer_logs = [l for l in logs if "continuing" in l.get("event", "")]
    assert outer_logs == [], "fail-fast path must not emit the 'continuing' outer log"
