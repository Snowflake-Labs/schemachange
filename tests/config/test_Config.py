...
from schemachange.config.get_merged_config import config_factory
...
def yaml_config(_) -> DeployConfig:
    return DeployConfig.factory(
        config_folder=Path(__file__).parent.parent.parent / "demo" / "basics_demo",
        ...
        change_history_table=ChangeHistoryTable(table_name="yaml_change_history_table"),
...
class TestConfig:
    ...
    def test_inject_config_file_path_no_config_folder(self, _):
        config = BaseConfig.factory(subcommand="deploy")
    ...
    def test_inject_config_file_path_with_config_folder(self, _):
        ...
        config = BaseConfig.factory(subcommand="deploy", config_folder=config_folder)
        assert (
    ...
    def test_invalid_config_folder(self, _):
        ...
        assert "Path is not valid directory: some_config_folder_name" in e_info_value

    @mock.patch("pathlib.Path.is_dir", side_effect=[True, False, False])
    ...
    @mock.patch("pathlib.Path.is_dir", side_effect=[True, True, False])
    ...
    def test_config_vars_not_a_dict(self):
        with pytest.raises(Exception) as e_info:
            BaseConfig.factory(subcommand="deploy", config_vars="a string")
        assert (
    ...
    def test_config_vars_reserved_word(self):
        with pytest.raises(Exception) as e_info:
            BaseConfig.factory(
                subcommand="deploy", config_vars={"schemachange": "not allowed"}
            )
        assert (
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                DeployConfig.factory(
                    config_folder=Path(__file__).parent.parent.parent
                    / "demo"
                    / "citibike_demo",
                    root_folder=Path(__file__).parent.parent.parent
                    / "demo"
                    / "citibike_demo",
                    modules_folder=Path(__file__).parent.parent.parent
                    / "demo"
                    / "citibike_demo",
                    config_vars={"var1": "cli_vars"},
                    ...
                    change_history_table=ChangeHistoryTable(
                        table_name="cli_change_history_table"
                    ),
    ...
    def test_merge_exclude_unset(
        ...
        merged_config = yaml_config.merge_exclude_defaults(other=cli_config)

        for name, field in dataclasses.asdict(merged_config).items():
            if name not in cli_overrides:
                expected = getattr(yaml_config, name)
            else:
                expected = getattr(cli_config, name)

            if isinstance(expected, Path):
                assert str(getattr(merged_config, name)) == str(expected)
            else:
                assert getattr(merged_config, name) == expected
