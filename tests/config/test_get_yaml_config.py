...
from schemachange.config.get_merged_config import get_yaml_config
...
def test_get_yaml_config(_):
    config_file_path = Path(__file__).parent / "schemachange-config-complete.yml"
    ...
    assert yaml_config.change_history_table == ChangeHistoryTable(
    ...
    assert yaml_config.config_vars == {"var1": "from_yaml"}
